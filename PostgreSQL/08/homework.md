...
Для `autovacuum` предусмотрено 15 параметров:
```console
postgres=# SELECT name, setting, unit, short_desc
FROM pg_settings WHERE name like '%autovacuum%';
                 name                  |  setting  | unit |                                        short_desc
---------------------------------------+-----------+------+-------------------------------------------------------------------------------------------
 autovacuum                            | on        |      | Запускает подпроцесс автоочистки.
 autovacuum_analyze_scale_factor       | 0.1       |      | Отношение числа добавлений, обновлений или удалений кортежей к reltuples, определяющее потребность в анализе.
 autovacuum_analyze_threshold          | 50        |      | Минимальное число добавлений, изменений или удалений кортежей, вызывающее анализ.
 autovacuum_freeze_max_age             | 200000000 |      | Возраст, при котором необходима автоочистка таблицы для предотвращения зацикливания ID транзакций.
 autovacuum_max_workers                | 3         |      | Задаёт предельное число одновременно выполняющихся рабочих процессов автоочистки.
 autovacuum_multixact_freeze_max_age   | 400000000 |      | Возраст multixact, при котором необходима автоочистка таблицы для предотвращения зацикливания multixact.
 autovacuum_naptime                    | 60        | s    | Время простоя между запусками автоочистки.
 autovacuum_vacuum_cost_delay          | 2         | ms   | Задержка очистки для автоочистки (в миллисекундах).
 autovacuum_vacuum_cost_limit          | -1        |      | Суммарная стоимость очистки, при которой нужна передышка, для автоочистки.
 autovacuum_vacuum_insert_scale_factor | 0.2       |      | Отношение числа добавлений кортежей к reltuples, определяющее потребность в очистке.
 autovacuum_vacuum_insert_threshold    | 1000      |      | Минимальное число добавлений кортежей, вызывающее очистку; при -1 такая очистка отключается.
 autovacuum_vacuum_scale_factor        | 0.2       |      | Отношение числа обновлений или удалений кортежей к reltuples, определяющее потребность в очистке.
 autovacuum_vacuum_threshold           | 50        |      | Минимальное число изменений или удалений кортежей, вызывающее очистку.
 autovacuum_work_mem                   | -1        | kB   | Задаёт предельный объём памяти для каждого рабочего процесса автоочистки.
 log_autovacuum_min_duration           | -1        | ms   | Задаёт предельное время выполнения автоочистки, при превышении которого эта операция протоколируется в журнале.
(15 rows)
```
Узнаем значение по умолчанию для параметра `autovacuum_vacuum_cost_limit`:
```console
               name                |  setting   | unit |                                     short_desc
-----------------------------------+------------+------+-------------------------------------------------------------------------------------
 vacuum_cost_limit                 | 200        |      | Суммарная стоимость очистки, при которой нужна передышка.
```
...
```
`pg_class.reltuples` = Число строк в таблице. Это лишь примерная оценка, используемая планировщиком.

Будем отталкиваться от стандартных параметров и пойдем в сторону агрессивности autovacuum – будем постепенно давать вакууму больше ресурсов, больше времени на работу, меньше интервалы запуска, более ранний порог срабатывания. Исключение: параметр cost_delay в качестве компенсаторного будем увеличивать.
Всего запланируем пять тестов (один уже был проведен выше):
| **Параметр**  | **#1** |**#2**|**#3**|**#4**|**#5**|
| ------  |- |-|-|-|-|
| autovacuum_max_workers|3|5|7|9|11|
| autovacuum_naptime|60s|40s|30s|15s|10s|
| autovacuum_vacuum_threshold|50|40|30|20|10|
